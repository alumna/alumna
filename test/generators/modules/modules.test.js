...
import modules from './../../../src/generators/app/modules';


/* Mocking directories */
modules.__Rewire__( 'base_dir', __dirname );
modules.__Rewire__( 'modules_dir', __dirname + '/modules/' );
...
test('Modules - Case 1 - Missing options.modules', () => {
	...
	return modules( options ).then( code => {
...
test('Modules - Case 2 - options.modules isn\'t an object', () => {
	...
	return modules( options ).catch( error => {
...
test('Modules - Case 6 - Module using module.hjson file', () => {
	...
	return modules( options ).then( code => {
		...
		expect( code ).toBe( 'The browser code.' + EOL + EOL + 'Altiva.modules = {};' + EOL + EOL + 'Altiva.modules[ \'test6\' ] = ' + '{ do: \'this\' }' + EOL + EOL );
...
test('Modules - Case 7 - Module using package.json file', () => {
	...
	return modules( options ).then( code => {
		...
		expect( code ).toBe( 'The browser code.' + EOL + EOL + 'Altiva.modules = {};' + EOL + EOL + 'Altiva.modules[ \'test7\' ] = ' + '{ do: \'that\' }' + EOL + EOL );
...
test('Modules - Case 12 - Main file not found in project\'s directory', () => {
	...
	let promise = modules( options ).catch( error => {
